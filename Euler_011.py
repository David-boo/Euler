...
# Code could be improvable but it works (to-do)
# We will be using numpy package , fundamental for scientific computing on Python (check more information about numpy and other interesting packages on david-boo.github.io) 

import numpy as np

# Define the grid and then, compute the product of 4 numbers in the 4 directions. If the result is the highest value so far, store it as max and print in the end.
...
for i in range(20):
    for j in range(17):
        ...
        if res >= max:
            max = res
        res = arr[j][i] * arr[j + 1][i] * arr[j + 2][i] * arr[j + 3][i]
        if res >= max:
            max = res
for i in range(3, 20):
    for j in range(17):
        ...
        if res >= max:
            max = res
        res = arr[j][i] * arr[j + 1][i - 1] * \
            arr[j + 2][i - 2] * arr[j + 3][i - 3]
        if res >= max:
            max = res
